...
foreach my $mt (@mtList) {
  foreach my $genome (@genomeList) {
    push @commandArray, '/usr/local/bin/htseq-count -f sam -r pos -s no -a 0 -t CDS -i locus_tag -m intersection-strict -o '. $countFolder.'/'.$mt.'-'.$genome.'.CDS.sam '.$mapFolder.'/'.$mt.'-'.$genome.'.sam '.$gffFolder.'/'.$genome.'.gff >  '.$countFolder.'/'.$mt.'-'.$genome.'.CDS.out';
  }
}
################################################################################
