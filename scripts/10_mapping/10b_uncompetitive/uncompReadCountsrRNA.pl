...
foreach my $genome (@genomeList) {
    ...
    foreach my $mt (@mtList) {
      ...
      # Call HTseq-count
      system('/usr/local/bin/htseq-count -f bam -r pos -s no -a 0 -t rRNA -i locus_tag -m intersection-strict -o '. $countFolder.'/'.$mt.'-'.$genome.'.rRNA.sam '.$mapFolder.'/'.$mt.'-'.$genome.'.sorted.bam '.$gffFolder.'/'.$genome.'.gff >  '.$countFolder.'/'.$mt.'-'.$genome.'.rRNA.out');
      }
  }
