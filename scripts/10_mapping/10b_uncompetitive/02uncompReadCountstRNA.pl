...
foreach my $genome (@genomeList) {
    ...
    foreach my $mt (@mtList) {
      ...
      # Call HTseq-count
      system('/usr/local/bin/htseq-count -f sam -r pos -s no -a 0 -t tRNA -i locus_tag -m intersection-strict -o '. $countFolder.'/'.$mt.'-'.$genome.'.tRNA.sam '.$mapFolder.'/'.$mt.'-'.$genome.'.sam '.$gffFolder.'/'.$genome.'.gff >  '.$countFolder.'/'.$mt.'-'.$genome.'.tRNA.out');
      }
  }
